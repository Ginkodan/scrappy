...
<div style="margin-bottom:0.6rem">
  {#if schemas.length === 0}
    <div style="color:#444;font-size:0.78rem;margin-bottom:0.5rem">No schemas yet.</div>
  {:else}
    {#each schemas as s}
      <div style="display:flex;align-items:center;justify-content:space-between;padding:0.35rem 0;border-bottom:1px solid #1a1a1a">
        <span style="font-size:0.8rem;color:#ccc">
          {s.display_name}
          <span style="color:#444;font-size:0.7rem">{s.id}</span>
        </span>
        <div style="display:flex;gap:0.4rem">
          <button
            style="all:unset;font-size:0.65rem;color:#666;background:#1e1e1e;border:1px solid #2a2a2a;border-radius:3px;padding:0.1rem 0.45rem;cursor:pointer"
